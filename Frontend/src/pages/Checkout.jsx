...
const Checkout = () => {
  ...
  const [orderSummary, setOrderSummary] = useState({ subtotal: 0, discount: 0, deliveryCharge: 50, total: 0 });
  ...
  useEffect(() => {
    ...
    const subtotal = storedCart.reduce((total, item) => total + item.price * (item.quantity || 1), 0);
      ...
      {cartItems.length === 0 ? (
        ...
        <div>
          <div className="mb-6">
            ...
            {cartItems.map(item => (
              <div key={item.id} className="flex justify-between items-center mb-4 p-4 border border-gray-700 rounded-lg">
                <div>
                  <h2 className="text-xl">{item.name}</h2>
                  <p className="text-gray-400">Price: ₹{item.price}</p>
                  <p className="text-gray-400">Quantity: {item.quantity}</p>
                  <p className="text-gray-400">Subtotal: ₹{(item.price * (item.quantity || 1)).toFixed(2)}</p>
            ...
            <div className="flex flex-col mt-6 space-y-4">
              ...
              <div className="flex justify-between">
                <span className="text-xl">Discount ({DISCOUNT_PERCENTAGE}%):</span>
          ...
          <div className="mb-6">
            ...
            <input
              ...
              className="w-full p-3 border border-gray-300 rounded bg-gray-800 text-gray-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            />
          </div>
